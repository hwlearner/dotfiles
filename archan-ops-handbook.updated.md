...
Codex / cc-connect 当前目录：

- `~/.codex -> ~/.config/codex`
- `~/.cc-connect -> ~/.config/cc-connect`

`cc-connect` 当前配置：

- 配置文件：`/home/han/.cc-connect/config.toml`
- 平台：Feishu
- agent：Codex
- 模式：`yolo`
- 仅允许指定的 Feishu 用户访问和执行管理命令
- 用户级 systemd 服务：`cc-connect-esl.service`
...
systemctl --user status esl-api.service
systemctl --user status esl-browser-sync.service
systemctl --user status cc-connect-esl.service
